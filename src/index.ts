...
const startAgents = async () => {
  ...
  try {
    for (const character of characters) {
      const [_, runtime] = await startAgent(
        character,
        directClient as DirectClient
      );
      const agentId = characters[0].name ?? "Agent";

      questionListenerHandler(runtime as AgentRuntime)
        .then((question) => {
          queryAgent(question, agentId)
            .then(answer=>{
              responseToVoice(runtime as AgentRuntime, answer)
